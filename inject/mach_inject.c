...
mach_error_t mach_copy_mem(
                           pid_t targetProcess,
                           const void * input,
                           size_t inputSize,
                           vm_address_t * remoteAddress)
{
    ...
    vm_address_t remoteData = (vm_address_t)NULL;
    if( !err ) {
        printf("Allocating memory\n");
        err = vm_allocate( remoteTask, &remoteData, inputSize, 1 );
        
        vm_protect(remoteTask, remoteData, inputSize, 0, VM_PROT_WRITE | VM_PROT_READ);
        
        if(!err) {
            printf("Wrote memory\n");
            err = vm_write( remoteTask, remoteData, (pointer_t) input, inputSize );
        }
    }
    if(remoteAddress != NULL && !err) {
        printf("Set memory return\n");
        *remoteAddress = remoteData;
    }
    return err;
}
mach_error_t
            ...
            vm_size_t                stackSize )
{
    ...
    if( !err && paramBlock != NULL && paramSize ) {
        ...
        if( !err ) {
            ...
            vm_address_t remoteParamBlock2 = remoteParamBlock + 0;
            //err = vm_write( remoteTask, remoteParamBlock,
            //               (pointer_t) &remoteParamBlock2, 8 );
            
            err = vm_write( remoteTask, remoteParamBlock2,
