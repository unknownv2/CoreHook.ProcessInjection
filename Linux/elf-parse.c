...
INJECT_EXPORT void print_elf(elf_rt_t *target)
{
...
INJECT_EXPORT bool parse_elf(elf_rt_t *target)
{
    parse_header(target);
    ...
    find_symbol_in_lib(dyn_info_t *dyldinfo, elf_rt_input_t input, char *sym_name)
{
    ...
    //Xdebug("start bucket search...");
    do
    {
        elf_rt_read(input, hash_addr, &h2, sizeof(unsigned int));
        Xdebug("\th2: 0x%x(%u) = (%u) = [%u]\n", h2, h2, new_hash, (unsigned int)((h2 ^ new_hash)));
        /*1. hash value same */
        if (((h2 ^ new_hash) >> 1) == 0)
        {
...
INJECT_EXPORT long find_symbol(elf_rt_t *target, char *sym_name, char *lib_name)
{
